...
def test_datastream_initialization(sample_datastream_obj: DataStream.DataStream) -> None:
...
def test_raises_value_error_if_offset_less(sample_datastream_obj: DataStream.DataStream) -> None:
    ...
    onset_timestamp = np.random.uniform(low = 1.0, high = sample_datastream_obj.data.item(-1, 'time_stamp'))
...
def test_raises_value_error_if_offset_onset_equal(
    ...
    onset_timestamp = offset_timestamp = np.random.uniform(low = 0, high = sample_datastream_obj.data.item(-1, 'time_stamp'))
    with pytest.raises(
        ValueError, match = "Offset timestamp must be greater than onset timestamp."
        ):
        sample_datastream_obj.filter_time_range(onset_timestamp, offset_timestamp)
...
def test_raises_value_error_if_timestamps_negative(
    ...
    onset_timestamp = np.random.uniform(low = -1000, high = -2.0) # idk if its necessary for low to be from the df or if it can be an arbitrary number
